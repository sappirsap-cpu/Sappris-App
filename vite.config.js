    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          {
            // Supabase — network-only, never cache (critical for auth/data)
            urlPattern: /^https:\/\/.*\.supabase\.co\/.*/i,
            handler: 'NetworkOnly',
          },
          {
            // Netlify API functions (ai-proxy) — network-only
            urlPattern: /\/api\/.*/i,
            handler: 'NetworkOnly',
          },
          {
        ...
        // Don't cache navigate requests — always fetch fresh HTML
